...
class LLMAnalysis:
    ...
    def chat_interaction(self, title=None, abstract=None):
        ...
        print(self.prompt['prompt_header'])
        input_ai = (
    ...
    def llm_analysis(self, data=None, task="remain", json_path="./", json_file="Articles_llm.json"):
        """
        ...
        try:
            ...
            data_llm = pd.DataFrame(columns=['id'])

        # Ensure all columns from `data` exist in `data_llm`
        ...
        for index, row in data.iterrows():
            ...
            if row["id"] in data_llm["id"].values:
                # Update existing row
                data_llm.loc[data_llm["id"] == row["id"], list(new_entry.keys())] = list(new_entry.values())
            else:
                # Append new row
